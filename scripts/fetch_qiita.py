#!/usr/bin/env python3
"""
fetch_qiita.py - Fetch beauty/wellness-related articles from Qiita via Qiita API v2.

Usage:
    python scripts/fetch_qiita.py --year 2025 --output /tmp/qiita_raw.json
    python scripts/fetch_qiita.py --year 2026 --output /tmp/qiita_raw.json  # auto past-12-months
...
# Qiita search queries for beauty/wellness topics (Japanese-primary)
BEAUTY_QUERIES = [
    "スキンケア アプリ",
    "美容 アプリ開発",
    "コスメ 分析",
...
def compute_date_range(target_year: int) -> tuple[datetime, datetime]:
    ...
    if target_year >= current_year:
        # Year not yet complete → past 12 months
        end_dt = now
...
def fetch_qiita_articles(query: str, start_dt: datetime, end_dt: datetime,
    ...
    # Qiita search query with date filter
    # Format: YYYY-MM-DDTHH:MM:SS+09:00 (Qiita uses JST for created filter)
    created_filter = (
    ...
    headers = {
        "User-Agent": "BeautyIdeaResearch/1.0",
...
def main():
    parser = argparse.ArgumentParser(description="Fetch beauty-related Qiita articles for idea research")
